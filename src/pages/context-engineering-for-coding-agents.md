...
<script type="application/ld+json">{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"Is context engineering just longer prompts?","acceptedAnswer":{"@type":"Answer","text":"No. It is the same information, but structured and placed where it belongs — in the repository — rather than stuffed into a prompt. Durable context belongs in the repo, not the prompt."}},{"@type":"Question","name":"What context should live in the repo?","acceptedAnswer":{"@type":"Answer","text":"At minimum: what the project is, what the architectural boundaries are, which commands prove correctness, and what not to touch. Start with 5 AGENTS.md sections and expand from there as failure cases appear."}},{"@type":"Question","name":"Can too much context hurt?","acceptedAnswer":{"@type":"Answer","text":"Yes. If AGENTS.md is more than a screen of text, cut detail and link to deeper docs. Agents have limited attention too. The goal is useful, not complete."}},{"@type":"Question","name":"How should context be maintained?","acceptedAnswer":{"@type":"Answer","text":"Treat repo context like code: it is owned, it has tests (the validation loop), and it gets updated when failure cases are discovered. When an agent makes a mistake twice, update the repo."}},{"@type":"Question","name":"What is the minimum context needed for a small repo?","acceptedAnswer":{"@type":"Answer","text":"Even a small repo benefits from three things: an AGENTS.md file with project overview and validation commands, a one-paragraph architecture note, and a list of safety boundaries (what not to touch). That is enough to prevent the most common agent failure modes."}},{"@type":"Question","name":"How does context engineering differ from RAG?","acceptedAnswer":{"@type":"Answer","text":"RAG (Retrieval-Augmented Generation) pulls documents into the prompt at runtime. Context engineering places durable context in the repository itself so agents encounter it naturally without runtime retrieval. Both can be used together — RAG for dynamic information, repo context for stable operating knowledge."}},{"@type":"Question","name":"Can context engineering help with multi-agent workflows?","acceptedAnswer":{"@type":"Answer","text":"Yes. When multiple agents work in the same repo, shared repo context (AGENTS.md, decision records, validation matrix) ensures all agents operate from the same constraints. This is especially valuable for repos where different agents handle different subsystems."}}]}</script>
...
- [Coding-Agent Failure Modes: A Diagnostic Taxonomy](/blog/coding-agent-failure-modes/) — classify the first wrong decision and choose the right repair layer
